...
| 서버 호출 | 업로드 큐(Pending → Uploading → Completed) | **직접 서버 API 호출** (큐 미사용) |
| 큐 상태 UI | 표시 (Pending/Uploading/Failed 아이콘) | **미표시** |
| 완료 피드백 | 업로드 완료 Push Notification | **인앱 토스트/스낵바** (푸시 알림 불필요) |
| 위치(도면 좌표) 변경 | N/A | **불가** — 콘텐츠(텍스트·미디어)만 수정 |

**편집에 큐 미사용 근거**: 등록은 오프라인 지원을 위해 큐가 필요하지만, 편집은 사용자가 화면을 직접 보고 있는 온라인 전제 작업이다. 큐를 거치면 오히려 편집 완료 시점이 불명확해지며, 즉시 반영이 UX상 더 자연스럽다.
...
- ✅ 저장 성공 → 인앱 토스트/스낵바: `"Thread reply updated"` (i18n 키 별도 정의)
- ❌ 저장 실패 → 에러 토스트 + 편집 상태 유지 (입력 내용 소실 없음)
- 📌 **Push Notification 불필요**: 편집 시 사용자는 화면을 직접 보고 있는 상태. 등록 완료 알림(앱 백그라운드 가정)과 맥락이 다르므로 푸시는 발송하지 않음.
...
| 7 | ~~스레드 편집도 큐 상태 UI(Pending/Uploading/Completed)를 표시해야 하는가~~ | ✅ **편집은 큐 미사용 · 즉시 서버 호출** — 온라인 전제 작업이므로 큐 불필요, 인앱 로딩 인디케이터만 표시 (2026-06-05) |
